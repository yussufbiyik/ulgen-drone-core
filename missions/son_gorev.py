# 3.1. Görev Senaryosu
# Bu çalışmada test edilen uçuş senaryosu aşağıdaki adımlardan oluşmaktadır:
# •	Kalkış ve Formasyon Kurma: Tüm İHA’lar belirlenen irtifaya otomatik kalkış yapar ve dağıtık atama algoritmasıyla seçilen formasyonu oluşturur.
# •	Navigasyon Noktası 1: İHA sürüsü, formasyonu koruyarak ilk navigasyon noktasına hareket eder.
# •	Formasyon Değişimi: İlk noktada, kullanıcı arayüzü üzerinden yeni bir formasyon seçilebilir. İHA’lar dağıtık atama algoritması ile yeniden organize olur ve yeni formasyonu korumaya başlar.
# •	Navigasyon Noktası 2: Yeniden oluşturulan formasyon korunarak ikinci navigasyon noktasına hareket edilir.
# •	İHA Çıkarma ve Yedek İHA Ekleme: İkinci noktada sürüden bir İHA görev dışı bırakılır (eve dönüş). Ardından yerde bekleyen yedek İHA boşalan pozisyona atanır ve sürü tekrar tam sayıda devam eder.
# •	Navigasyon Noktası 3 ve İniş: Yedek İHA’nın katılımıyla sürü üçüncü navigasyon noktasına gider ve formasyonu bozmadan koordineli iniş yapar.
# Bu senaryo hem simülasyon ortamında (PX4-SITL + Gazebo) hem de gerçek uçuş testlerinde başarıyla uygulanmıştır. Böylece önerilen dağıtık algoritmanın hem sanal hem de gerçek koşullarda çalışabilirliği doğrulanmıştır.
import sys
import logging
import asyncio
import time
